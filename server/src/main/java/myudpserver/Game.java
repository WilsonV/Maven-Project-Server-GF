...
public class Game implements Runnable {
                    ...
                    try {
                        ...
                        try {
                            ...
                            FileWriter fr = new FileWriter(
                                    new File("Users/" + i.account + "/Character/" + i.name + "/rp.con"), false);
                            fr.write(rp + "\n");
                            fr.write(wins + "\n");
                            fr.write(losses + "\n");
                            fr.close();
                ...
                for (Player j : players) {
                    ...
                    if (i != j) { // do not send to self
                        if (j.connection_status.equals("lost") && (current_time - j.last_disconnected_time >= 1000)) {
                            ...
                            i.message(socket, send_buf);
                            j.last_disconnected_time = System.currentTimeMillis(); // wait 1 sec before sending next
                                                                                   // disconnect time
                        }
                    }
                }
    ...
    /*
     * private void messageSender(DatagramSocket socket,byte[] buf,InetAddress
     * address,int port){
     * 
     * try{
